...
export const InlineHeader = (
      ...
      const widthProperties = optToolbarWidth.map(
        (toolbarWidth: number) => {
          ...
          */
          ...
          // Despite how much availableWidth there is, never go smaller than minimumWidth and never go larger than the original toolbarWidth.
  ...
  const updateChromeUi = (resetDocking: boolean = false) => {
    ...
    // TINY-7827: This width can be used for calculating the "width" when resolving issues with flex-wrapping being triggered at the window width, despite scroll space being available to the right.
    ...
    We may need to calculate the width again after this, also - just be aware of that.
    */
    if (isSplitToolbar) {
    ...
    // Positioning
    if (!useFixedToolbarContainer) {
      // This will position the container in the right spot. Due to TINY-7827, it can set the "right" as well as the "left".
